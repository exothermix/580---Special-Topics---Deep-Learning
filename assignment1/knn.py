...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
      x_train_flat = x_train[i].view(-1)
      x_test_flat = x_test.view(num_test, -1)
      sub = torch.sub(x_test_flat, x_train_flat)
      dists[i] = torch.sum(sub**2, dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flat = x_test.view(num_test, -1)
    dists = -2 * torch.mm(x_train_flat, x_test_flat.t()) + torch.sum(x_test_flat**2, dim=1) + torch.sum(x_train_flat**2, dim=1).view(-1, 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        ...
        new_train_label = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i], dim=0)
        classifier = KnnClassifier(new_train, new_train_label)
        k_to_accuracies[k].append(classifier.check_accuracy(new_val, new_val_label, k=k))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
      mean = statistics.mean(k_to_accuracies[k])
      if mean > acc_best:
        acc_best = mean
        best_k = k
    ##########################################################################
