...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    even_rows_odd_cols = x[0:3:2, 1:5:2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x[0,:] = torch.tensor([0, 1, 2, 2, 2, 2, 0])
    x[1,:] = torch.tensor([0, 1, 2, 2, 2, 2, 0])
    x[2,:] = torch.tensor([3, 4, 3, 4, 5, 5, 0])
    x[3,:] = torch.tensor([3, 4, 3, 4, 5, 5, 0])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    rows = len(x) 
    columns = 1 + max(x)
    y = torch.zeros((rows, columns), dtype=torch.float32)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sd = torch.std(y, dim=0)
    y = torch.div(torch.sub(y, mean), sd)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
